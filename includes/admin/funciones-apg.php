...
function apg_city_enlaces( $enlaces, $archivo ) {
	...
	if ( $archivo === DIRECCION_apg_city ) {
		$enlaces[] = '<a href="' . esc_url( $apg_city['donacion'] ) . '" target="_blank" title="' . esc_attr__( 'Make a donation by ', 'wc-apg-city' ) . 'APG"><span class="genericon genericon-cart"></span></a>';
		$enlaces[] = '<a href="' . esc_url( $apg_city['plugin_url'] ) . '" target="_blank" title="' . esc_attr( $apg_city['plugin'] ) . '"><strong class="artprojectgroup">APG</strong></a>';
		$enlaces[] = '<a href="https://www.facebook.com/artprojectgroup" title="' . esc_attr__( 'Follow us on ', 'wc-apg-city' ) . 'Facebook" target="_blank"><span class="genericon genericon-facebook-alt"></span></a> <a href="https://x.com/artprojectgroup" title="' . esc_attr__( 'Follow us on ', 'wc-apg-city' ) . 'X" target="_blank"><span class="genericon genericon-x-alt"></span></a> <a href="https://es.linkedin.com/in/artprojectgroup" title="' . esc_attr__( 'Follow us on ', 'wc-apg-city' ) . 'LinkedIn" target="_blank"><span class="genericon genericon-linkedin"></span></a>';
		...
		$enlaces[] = '<a href="mailto:info@artprojectgroup.es" title="' . esc_attr__( 'Contact us by ', 'wc-apg-city' ) . 'e-mail"><span class="genericon genericon-mail"></span></a>';
...
function apg_city_enlace_de_ajustes( $enlaces ) {
	...
	$enlaces_de_ajustes = [
		'<a href="' . esc_url( admin_url( $apg_city['ajustes'] ) ) . '" title="' . esc_attr( __( 'Settings of ', 'wc-apg-city' ) . $apg_city['plugin'] ) . '">' . esc_html__( 'Settings', 'wc-apg-city' ) . '</a>',
		'<a href="' . esc_url( $apg_city['soporte'] ) . '" title="' . esc_attr( __( 'Support of ', 'wc-apg-city' ) . $apg_city['plugin'] ) . '">' . esc_html__( 'Support', 'wc-apg-city' ) . '</a>',
	];
...
function apg_city_plugin( $nombre ) {
	...
	$titulo = esc_attr( sprintf( __( 'Please, rate %s:', 'wc-apg-city' ), $apg_city['plugin'] ) );
	$url    = esc_url( $apg_city['puntuacion'] . '?rate=5#postform' );
	...
	if ( false === $respuesta ) {
		...
		// Un error no se cachea 24 horas: se reintenta en la siguiente hora.
		set_transient( 'apg_city_plugin', $respuesta, is_wp_error( $respuesta ) ? HOUR_IN_SECONDS : DAY_IN_SECONDS );
	}

	if ( is_wp_error( $respuesta ) ) {
		return '<a title="' . $titulo . '" href="' . $url . '" class="estrellas">' . esc_html__( 'Unknown rating', 'wc-apg-city' ) . '</a>';
	}
	...
	return '<a title="' . $titulo . '" href="' . $url . '" class="estrellas">' . $estrellas . '</a>';
}
 ...
 *
 * @param string $hook Hook de pantalla actual en el admin.
 *
 * @return void
 */
function apg_city_estilo( $hook ) {
	$pantalla_ajustes = ( 'woocommerce_page_wc-apg-city' === $hook );

	// Antes se miraba REQUEST_URI; el hook de pantalla identifica la página sin leer la petición.
	if ( ! $pantalla_ajustes && 'plugins.php' !== $hook ) {
		return;
	}

	wp_enqueue_style( 'apg_city_hoja_de_estilo', plugins_url( 'assets/css/style.css', DIRECCION_apg_city ), [], VERSION_apg_city, 'all' );

	if ( $pantalla_ajustes ) {
		wp_enqueue_script( 'apg_city_admin', plugins_url( 'assets/js/apg-city-admin.js', DIRECCION_apg_city ), [ 'jquery' ], VERSION_apg_city, true );
	}
}
